    ...
    <aside className="w-56 shrink-0 bg-asphalt text-canvas min-h-screen flex flex-col justify-between">
      <div>
        <div className="px-6 py-6">
          ...
          <p className="text-xs font-mono text-steel mt-1">SCHOOL OWNER</p>
        </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${
                  isActive
                    ? 'bg-signal text-asphalt font-semibold'
                    : 'text-steel hover:text-canvas hover:bg-white/5'
                }`}
              >
                <Icon size={18} />
                {item.label}
      ...
      <div className="px-3 pb-6">
        <Link
          to="/"
          className="flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-steel hover:text-canvas hover:bg-white/5 transition-colors"
        >
          <LogOut size={18} />
          Log out
        </Link>
